...
object Day06_1 extends App with Day06 {
  val result = timeAndRecords.map { (time, record) =>
    (0 until time).map { speed =>
      speed * (time - speed)
    }.count(_ > record)
  }.product
...
object Day06_2 extends App with Day06 {
  ...
  def calcDistance(time: Long, speed: Long, accDistances: Seq[Long]): Seq[Long] = {
    ...
    if (distance <= record) {
      if (accDistances.nonEmpty && distance < accDistances.head) accDistances // Declining, we're done
